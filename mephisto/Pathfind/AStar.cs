...
namespace mephisto.Pathfind
{
    public class AStar
    {
        protected Tissue map;

        protected Point[] offset = new Point[4] {   new Point(0,-1), // up
										  new Point(0, 1), // down
										  new Point(-1,0), // left
										  new Point( 1,0)  // right
									  };

        protected AStarNode[][] nodeGrid;
        protected bool[][] openSet;
        protected bool[][] closedSet;
        ...
        protected MinHeap openList = new MinHeap(200 * 200);
		...
		protected float GetHCost(Point from, Point to)
		{
			int distx = from.X - to.X;
			int disty = from.Y - to.Y;
            ...
            return /*(int)*/ ((distx + disty) + (0.001f * (distx + disty)));
		}
        ...
        protected int GetHash(Point p)
		{
			return ( p.X * 199 ) + p.Y + p.X;
		}

		protected string GetCacheKey(Point from, Point to)
		{
			return (from.X.ToString() +
				    from.Y.ToString() +
				    to.X.ToString() +
				    to.Y.ToString());
		}
